...
$query = "SELECT * FROM videos WHERE category = 'english'";
$stmt = $pdo->query($query);
$englishVideos = $stmt->fetchAll(PDO::FETCH_ASSOC);
?>
...
<body>
    ...
    <div class="mvid">
        <h1>English Videos</h1>
        <?php if (!empty($englishVideos)): ?>
            <ul>
                <?php foreach ($englishVideos as $video): 
        ...
        <?php else: ?>
            <p style="text-align: center; font-size: 18px; color: #888;">No English videos found.</p>
        <?php endif; ?>
    </div>
    <script>
